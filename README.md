...
- **Индикатор статуса сбора** — зелёный/красный индикатор показывает, был ли последний сбор успешным.
- **Очистка истории** — доступна для всех серверов сразу или для каждого отдельно (в развёрнутой карточке сервера).
- **Расписание в шапке** — отображается дата следующего сбора, по клику — подробное описание расписания.
...
### Новые функции:
- Отображение расписания обновления в шапке страницы (следующая дата сбора, по клику — подробности).
- Индикатор ошибок сбора для каждого сервера (красный/зелёный кружок).
- Очистка истории для всех серверов и для отдельного сервера (в карточке).
- Кнопка смены темы перенесена в раздел «Управление».
### Улучшения:
- Переработан интерфейс: кнопки действий (история, очистка) теперь доступны только при развёрнутой карточке сервера.
- Автообновление страницы увеличено до 60 минут для снижения нагрузки.
- Модальное окно с расписанием открывается по клику на плашку в шапке.
- Встроенная справка дополнена информацией об очистке истории и удалении панели.
### Исправления:
- Исправлен автозапуск веб-сервера после перезагрузки роутера (добавлен `nohup` и корректные пути).
- Исправлена работа cron на Keenetic (использование `/opt/etc/cron.d/` вместо `crontab -l`).
...
- Работа с историей.
- Очистка истории (глобальная и для отдельных серверов).
